...
const CustomerDashboard = ({ onLogout }) => {
    ...
    const departments = [
        { name: 'Logistics', email: 'logistics@sccourier.com', phone: '+1 800 555 0101' },
        { name: 'Customer Support', email: 'support@sccourier.com', phone: '+1 800 555 0102' },
        { name: 'Billing', email: 'billing@sccourier.com', phone: '+1 800 555 0103' },
    ];
    ...
    return (
        <div className="dashboard-container" style={{ animation: 'fadeIn 0.5s ease', padding: '2rem', width: '100%', maxWidth: '1200px', margin: '0 auto', zIndex: 10 }}>
            <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
                ...
                <button className="secondary-btn" onClick={onLogout} style={{ width: 'auto', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
                ...
                <div className="action-card" style={{ width: '100%', maxWidth: '100%', padding: '2rem', animation: 'slideInRight 1s ease backwards 0.4s', margin: 0 }}>
                    <div className="card-header" style={{ marginBottom: '1.5rem', textAlign: 'left' }}>
                        <h3 style={{ fontSize: '1.4rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#fff' }}>
                            <i className='bx bx-buildings' style={{ color: 'var(--accent-color)' }}></i> Our Departments
                        </h3>
                    </div>
                    
                    <div className="departments-list" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {departments.map((dept, index) => (
                            <div key={index} className="department-item" style={{ 
                                padding: '1.25rem', 
                            ...
                            >
                                <h4 style={{ color: 'var(--text-primary)', marginBottom: '0.75rem', fontSize: '1.15rem' }}>{dept.name}</h4>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <i className='bx bx-envelope' style={{ color: 'var(--accent-color)', fontSize: '1.1rem' }}></i>
                                        </div>
                                        <span>{dept.email}</span>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <i className='bx bx-phone' style={{ color: 'var(--accent-color)', fontSize: '1.1rem' }}></i>
                                        </div>
